...
class P剑指OfferII106VEAB3K{
    public static void main(String[] args) {
        Solution solution = new P剑指OfferII106VEAB3K().new Solution();
        // TO TEST
...
class Solution {
        ...
        public final int Unkownm=0;
    public boolean isBipartite(int[][] graph) {
        int len=graph.length;
        int[] color=new int[len];
        Arrays.fill(color,Unkownm);
        for (int i = 0; i < len; i++) {
            if (color[i]==Unkownm){
                ...
                while (!queue.isEmpty()){
                    int node=queue.poll();
                    int ccol=color[node]==Red?Green:Red;
                    for (int j = 0; j < graph[node].length; j++) {
                        if (color[graph[node][j]]==Unkownm){
                            color[graph[node][j]]=ccol;
                            queue.offer(graph[node][j]);
                        }else if (color[graph[node][j]]!=ccol){
                            return false;
                        }
                    }
                }
