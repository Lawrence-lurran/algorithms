...
// Related Topics 数组 哈希表 动态规划 👍 16 👎 0
...
class P剑指OfferII093Q91FMA{
    public static void main(String[] args) {
        Solution solution = new P剑指OfferII093Q91FMA().new Solution();
        // TO TEST
    }
...
class Solution {
    public int lenLongestFibSubseq(int[] arr) {
        ...
        for (int i = 0; i < len; i++) {
            for (int j = 0; j < i; j++) {
                int old=index.getOrDefault(arr[i]-arr[j],-1);
                if (old>=0 && old<j){
                    int cand=longest.getOrDefault(old*len+j,2)+1;
