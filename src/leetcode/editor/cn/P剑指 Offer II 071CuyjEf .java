...
class P剑指OfferII071CuyjEf{
    public static void main(String[] args) {
        Solution solution = new P剑指OfferII071CuyjEf().new Solution(new int[]{1});
        // TO TEST
    }
...
class Solution {
    ...
    public int pickIndex() {
        ...
        return binarySerch(x);
    }
    public int binarySerch(int traget){
        int left=0;
        int right=pre.length;
        while (left<right){
            int mid=left+((right-left)>>1);
            if (pre[mid]==traget){
                right=mid;
            }else if (pre[mid]<traget){
                left=mid+1;
            }else if (pre[mid]>traget){
                right=mid;
            }
