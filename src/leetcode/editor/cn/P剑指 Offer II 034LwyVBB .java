...
class P剑指OfferII034LwyVBB{
    public static void main(String[] args) {
        Solution solution = new P剑指OfferII034LwyVBB().new Solution();
        // TO TEST
    ...
    public boolean isAlienSorted(String[] words, String order) {
        ...
        for (int i = 0; i < words.length; i++) {
            for (int j = i+1; j < words.length; j++) {
                ...
                for (int i1 = 0; i1 < len;) {
                    if (orderMap.get(words[i].charAt(i1) )> orderMap.get(words[j].charAt(i1))){
                        return false;
                    }else if (orderMap.get(words[i].charAt(i1) )== orderMap.get(words[j].charAt(i1))){
                        i1++;
                        if (i1==len){
                            if (leni>lenj){
                                return false;
